...
3. Write `.scratch/index.md`: a table with one row per directory, sorted by stage in vocabulary order (`charting`, `steering`, `spec`, `tickets`, `active`, `blocked`, `done`) then by name, each row linking the project. Completion: row count equals directory count.
