module Gambol.Server.Tests.Issue46MailboxHistoryDurabilityTests
...
let ``File mixed hello survives dispose create`` () = task {
    let dir = newTempDir ()
    let secret = Credential "issue-46-file-hello"
...
let ``Db mixed hello survives dispose create`` () = task {
    ...
    let secret = Credential "issue-46-db-hello"
    ...
    let second = hostDb connStr (Credential "issue-46-db-hello-2")
    try
...
let ``Undo after File restart fills Change and rejects ActorStart`` () = task {
    let dir = newTempDir ()
    let secret = Credential "issue-46-undo"
    let first = hostFile dir secret
    try
