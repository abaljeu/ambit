...
let ``persistence exception is logged replied and mailbox survives`` () = task {
    ...
    let agent = FileAgent.createWithDependencies (admittedCredentials ()) dependencies dataDir
    try
        let! postResult =
...
let ``soft-fail log is not replayed into FileAgent state after restart`` () = task {
    ...
    let defaults = FileAgent.defaultDependencies dataDir
    let dependencies = { defaults with persistGraphOps = softFailPersist }
    let agent1 = FileAgent.createWithDependencies (admittedCredentials ()) dependencies dataDir
    try
        let! postResult =
    ...
    let agent2 = FileAgent.createWithDependencies (admittedCredentials ()) dependencies dataDir
    try
        let! state =
