...
let ``POST Change and inverse Changes return complete confirmations in request order``
    ...
    withClient backend (fun client -> task {
        ...
        let undo = Change.inverse (EventId.fromJson 1) (Guid.NewGuid()) change
        let redo = Change.inverse (EventId.fromJson 2) (Guid.NewGuid()) undo
...
let ``file backend large paste inverse total response is measured`` () = task {
    ...
    let inverse = Change.inverse (EventId.fromJson 1) (Guid.NewGuid()) paste
...
let ``POST changes SetText changes child text and bumps revision`` (backend: BackendKind) =
    withClient backend (fun client -> task {
        ...
        let change = { id = EventId.fromJson 1; submissionId = Guid.NewGuid(); ops = [ Op.SetText(childId, "", "hello") ] }
...
let ``POST changes with bad op returns 400`` (backend: BackendKind) =
    withClient backend (fun client -> task {
        ...
        let change = { id = EventId.fromJson 0; submissionId = Guid.NewGuid(); ops = [ Op.SetText(bogusId, "wrong", "new") ] }
        let! resp = postChange client testFile change
        Assert.Equal(HttpStatusCode.BadRequest, resp.StatusCode)
        let! body = resp.Content.ReadAsStringAsync()
...
let ``POST changes twice bumps revision to 2`` (backend: BackendKind) =
    withClient backend (fun client -> task {
        ...
        let change2 = { id = EventId.fromJson 1; submissionId = Guid.NewGuid(); ops = [ Op.SetText(childId, "first", "second") ] }
    ...
    withClient backend (fun client -> task {
        ...
        let! _ =
            postChange client testFile
                { id = EventId.fromJson 1; submissionId = Guid.NewGuid(); ops = [ Op.SetText(childId, "", "persisted") ] }
...
let ``New server uses snapshot + log replay`` () = task {
    ...
    let! _ = postChange client1 testFile { id = EventId.fromJson 1; submissionId = Guid.NewGuid();
