...
[<Fact>]
let ``classifyDeleteForSelection promotes when the only other appearance is a self-Ref`` () =
...
[<Fact>]
let ``owned Delete with self-Ref: History rejects; client apply leaves self-owner`` () =
    let graph, x = ownedWithSelfRef ()
    let range = rootRange graph 0 1
    let classified = ViewModelDeleteOps.classifyDeleteForSelection graph range
    let ops = ViewModelDeleteOps.planDeleteOps graph range classified
    ...
    | ApplyResult.Unchanged _ -> Assert.True(false, "History should reject, not Unchanged")
    | ApplyResult.Changed _ ->
        Assert.True(false, "History should reject a self-owner cycle")
    match ResidentProjection.applyChange change (stateOf graph) with
    | ApplyResult.Invalid(_, msg) ->
        Assert.True(false, $"client apply should succeed locally: {msg}")
    | ApplyResult.Unchanged _ -> Assert.True(false, "Expected client Changed")
    | ApplyResult.Changed s ->
        Assert.Equal(Some x, Map.tryFind x s.graph.ownerParentByChild)
