...
[<Fact>]
let ``delete owned file with refs removes owner and refs`` () =
    let ws = NodeId.New()
    let file = NodeId.New()
    ...
    | ApplyResult.Changed s ->
        ...
        | ApplyResult.Changed s2 ->
            Assert.Empty(s2.graph.nodes.[normal].children)
            Assert.True(
                s2.graph.nodes.[Graph.trashId].children
                |> List.exists (fun c -> c.id = file && c.ref = Ownership.Owner))
            Assert.True(Map.containsKey file s2.graph.nodes)
        | _ -> Assert.Fail("expected changed")
    | _ -> Assert.Fail("setup failed")

[<Fact>]
let ``delete owned special from trash with refs hard-deletes owner and refs`` () =
    let ws = NodeId.New()
    let file = NodeId.New()
    let normal = NodeId.New()
    let g0 = Graph.create ()
    ...
    match g1 with
    | ApplyResult.Changed s ->
        let wsChildren = s.graph.nodes.[ws].children
        let trashLen = s.graph.nodes.[Graph.trashId].children.Length
        let g2 =
            History.applyChange
                { id = 1
                  changeId = System.Guid.NewGuid()
                  ops =
                    [ Op.Replace(ws, 0, wsChildren, owned [ normal ])
                      Op.Replace(Graph.trashId, trashLen, [], owned [ file ]) ] }
                s
        match g2 with
        | ApplyResult.Changed s2 ->
            let range = trashRange s2.graph trashLen 1
            let classified = ViewModelDeleteOps.classifyDeleteForSelection s2.graph range
            Assert.Equal(1, classified.Length)
            Assert.Equal(ViewModelDeleteOps.OwnedSpecialDeleteHard, classified.[0].action)
            let ops = ViewModelDeleteOps.planDeleteOps s2.graph range classified
            let result = History.applyChange { id = 2; changeId = System.Guid.NewGuid(); ops = ops } s2
            match result with
            | ApplyResult.Changed s3 ->
                Assert.Empty(s3.graph.nodes.[normal].children)
                Assert.Empty(getAllOccurrences s3.graph file)
            | _ -> Assert.Fail("expected changed")
        | _ -> Assert.Fail("setup trash failed")
    | _ -> Assert.Fail("setup failed")

[<Fact>]
let ``statusForDelete warns when owned special delete removed refs`` () =
    let ws = NodeId.New()
    let file = NodeId.New()
    let normal = NodeId.New()
    let g0 = Graph.create ()
    ...
    | ApplyResult.Changed s ->
        ...
        | None -> Assert.Fail("expected status")
        | Some msg ->
            Assert.Equal(StatusKind.StatusWarn, msg.kind)
            Assert.Contains("1 ref", msg.text)
    | _ -> Assert.Fail("setup failed")
