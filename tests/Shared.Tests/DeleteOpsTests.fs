...
let ``planDeleteOps second Directory with same name as one already in TRASH applies`` () =
    ...
    let graph1 =
        match ChangeValidation.applyChange { id = EventId.fromJson 0; submissionId = System.Guid.NewGuid(); ops = ops1 } (stateOf graph0) with
    ...
    let result =
        ChangeValidation.applyChange { id = EventId.fromJson 1; submissionId = System.Guid.NewGuid(); ops = ops2 } (stateOf graph2)
    match result with
    | ApplyResult.Invalid(_, msg) -> Assert.True(false, $"Invalid: {msg}")
    | ApplyResult.Unchanged _ -> Assert.True(false, "Expected Changed")
    | ApplyResult.Changed s ->
