...
let ``planDeleteOps multi-sibling: change applies and both nodes land under TRASH`` () =
    ...
    let result = SpecialNodeTestHelpers.applyChange change (stateOf graph)
    match result with
...
let ``planDeleteOps promotion: ref promoted to owner, original owner row removed`` () =
    ...
    let change = SpecialNodeTestHelpers.changeEventZero "" ops
    let result = SpecialNodeTestHelpers.applyChange change (stateOf g3)
    match result with
    | ApplyResult.Invalid(_, msg) -> Assert.True(false, $"Invalid: {msg}")
    | ApplyResult.Unchanged _ -> Assert.True(false, "Expected Changed")
    | ApplyResult.Changed s ->
...
let ``planDeleteOps single node MoveToTrash still works`` () =
    ...
    let change = SpecialNodeTestHelpers.changeEventZero "" ops
    let result = SpecialNodeTestHelpers.applyChange change (stateOf g2)
    match result with
    | ApplyResult.Invalid(_, msg) -> Assert.True(false, $"Invalid: {msg}")
    | ApplyResult.Unchanged _ -> Assert.True(false, "Expected Changed")
    | ApplyResult.Changed s ->
...
let ``owned Delete with self-Ref moves to TRASH and History applies`` () =
    ...
    match ResidentProjection.applyOps (SpecialNodeTestHelpers.eventOps change) (stateOf graph) with
...
let ``owned Delete with self-Ref and another Ref promotes the other Ref`` () =
    ...
    let change = SpecialNodeTestHelpers.changeEventZero "" ops
    match SpecialNodeTestHelpers.applyChange change (stateOf graph3) with
    | ApplyResult.Invalid(_, msg) -> Assert.True(false, $"Invalid: {msg}")
    | ApplyResult.Unchanged _ -> Assert.True(false, "Expected Changed")
    | ApplyResult.Changed s ->
