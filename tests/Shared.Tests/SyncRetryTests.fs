...
let ``ClientTimeout retry waits grace period`` () =
    Assert.Equal(30_000, SyncRetry.retryDelayMs 1 SubmitNetworkErrorKind.ClientTimeout)
    Assert.Equal(30_000, SyncRetry.retryDelayMs 3 SubmitNetworkErrorKind.ClientTimeout)
...
let ``FetchFailed retry uses exponential backoff`` () =
    ...
    Assert.Equal(30_000, SyncRetry.retryDelayMs 10 SubmitNetworkErrorKind.FetchFailed)
