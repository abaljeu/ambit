    ...
    let cb =
        { topLevelIds = [oldId]
          nodes =
            Map.ofList
                [ oldId,
                  { Node.create oldId with text = "hello" } ] }
...
let ``buildPasteOpsFromClipboard remaps parent-child relationship`` () =
    ...
    let cb =
        { topLevelIds = [aId]
          nodes = Map.ofList
            [ aId,
              { Node.create aId with text = "a"; children = owned [ bId ] }
              bId,
              { Node.create bId with text = "b" } ] }
    let newTopIds, ops = buildPasteOpsFromClipboard cb
    let graph = applyOps ops (Graph.create ())
...
let ``buildPasteOpsFromClipboard multiple top-level nodes`` () =
    ...
    let cb =
        { topLevelIds = [id1; id2]
          nodes = Map.ofList
            [ id1,
              { Node.create id1 with text = "x" }
              id2,
              { Node.create id2 with text = "y" } ] }
    let newTopIds, ops = buildPasteOpsFromClipboard cb
