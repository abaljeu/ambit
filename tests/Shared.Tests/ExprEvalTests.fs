...
let ``bind concatenates left-to-right in order`` () =
    let left _ = [ nodeAnswer 1; nodeAnswer 2 ]
    let right input =
        match input with
        | ExprAnswer.Node n when n.id = nodeId 1 -> [ nodeAnswer 10 ]
        | ExprAnswer.Node n when n.id = nodeId 2 -> [ nodeAnswer 20; nodeAnswer 21 ]
        | _ -> []
    assertNodeIds [ 10; 20; 21 ] (ExprEval.bind left right (nodeAnswer 0))

// ---- OR ----

[<Fact>]
let ``OR concatenates operand sequences and may repeat`` () =
    let left _ = [ nodeAnswer 1 ]
    let right _ = [ nodeAnswer 1; nodeAnswer 2 ]
    assertNodeIds [ 1; 1; 2 ] (ExprEval.orEval left right (nodeAnswer 0))

// ---- AND ----

[<Fact>]
let ``AND keeps left order with at-most-once intersection`` () =
    let left _ = [ nodeAnswer 1; nodeAnswer 2; nodeAnswer 1; nodeAnswer 3 ]
    let right _ = [ nodeAnswer 3; nodeAnswer 2; nodeAnswer 1 ]
    assertNodeIds [ 1; 2; 3 ] (ExprEval.andEval left right (nodeAnswer 0))

[<Fact>]
let ``AND intersects Text answers by string equality`` () =
    let left _ = [ textAnswer "a"; textAnswer "b"; textAnswer "a" ]
    let right _ = [ textAnswer "b"; textAnswer "a" ]
    let result = ExprEval.andEval left right (nodeAnswer 0)
    Assert.Equal<string list>([ "a"; "b" ], result |> List.choose (function ExprAnswer.Text t -> Some t | _ -> None))
...
let ``NOT yields input when operand is empty`` () =
    ...
    Assert.Equal<ExprAnswer list>([ input ], ExprEval.notEval inner input)

[<Fact>]
let ``NOT yields nothing when operand succeeds`` () =
    let inner _ = [ nodeAnswer 1 ]
    Assert.Equal<ExprAnswer list>([], ExprEval.notEval inner (nodeAnswer 0))
...
let ``catalog stub row registers and invokes through core`` () =
    ...
    Assert.Equal<ExprAnswer list>([ input ], ExprCatalog.invoke ExprBoundSlot.NoArgument found input)
...
let ``child answers on Unloaded node yields empty sequence`` () =
    ...
    Assert.Equal<ExprAnswer list>([], ExprWalk.childAnswers graph parent)
...
let ``child answers on Loaded node yields Children in order`` () =
    ...
    let graph =
        ...
        |> fun g ->
            ...
            Graph.fromNodes g.root nodes
    let result = ExprWalk.childAnswers graph parent
