...
The domain model for what makes agent documents predictable. A skill (or any agent-facing doc) exists to wrangle determinism out of a stochastic system; the root virtue is **Predictability**, and every term below is a lever on it. This is the disclosed reference for [`writing-for-agents`](SKILL.md).
