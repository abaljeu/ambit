...
**Context:** Parse already fits the long-running Actor shape as an observation, but still compare-and-swaps on revision and returns a bare ack. Realignment is proposed. This ticket is the tracer that proves [[plan/core-creation/issues/01-generalized-server-actor-produce-path.md]] without inventing multi-job identity, cancel, or soft-lock chrome.
