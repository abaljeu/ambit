...
Planners send **full lists** for `oldList` and `newList` on the wire. Prefer **one Replace per parent per Change**; compose multiple edits on one parent at plan time. Mailbox Append expands to this full-list Replace at execute; History stores Replace only, with initiating command Append on `Ev.commandName` ([[../issues/16-append-op-expands-to-replace.md]]).
