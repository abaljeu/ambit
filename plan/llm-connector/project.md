...
- 2026-09-25 — Coded [26 — Named AiKeys win over later JSON](issues/26-apikey-named-keys-provider-order.md): environment and Development user secrets win after extra JSON; `AiKeys:cursor` / `AiKeys__cursor` / `DefaultAiKey=cursor`. Status `coded`. Stage stays `done`.
...
- [26 — Named AiKeys win over later JSON](issues/26-apikey-named-keys-provider-order.md) — Status `coded`. Follow-up: `AiKeys:cursor` plus `DefaultAiKey=cursor` win over later empty JSON; no `AiKeys:0:ApiKey` fallback.
