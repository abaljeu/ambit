...
- [23 — Focus stream commit on next <](issues/23-focus-stream-commit-on-next-open.md) — Status `coded`. Follow-up: commit pending when the next `<` arrives; incomplete tag bytes stay in `hold`.
