...
- 2026-09-20 — Coded [15 — AiKeys from appsettings](issues/15-aikeys-from-appsettings.md): `AiKeys` bind/resolve; first entry default; `?ai` keyname; `CURSOR_API_KEY` removed. Status `coded`. Stage stays `done`.
...
- [15 — AiKeys from appsettings](issues/15-aikeys-from-appsettings.md) — Status `coded`. Follow-up: Server `AiKeys` from appsettings → `RunnerConfig.ApiKey`; remove `CURSOR_API_KEY` from Run Agent Actor.
