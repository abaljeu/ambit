...
- `appsettings.Development.json` is gitignored like Production. Base `appsettings.json` holds empty `AiKeys` / `AiRepos` placeholders. `addAppSettings` load order is unchanged.
...
1. [20 — Gitignore Development appsettings](issues/20-gitignore-development-appsettings.md) — Status `done`.
2. [19 — AI extract pack is XML with Focus cssClass](issues/19-ai-xml-pack-focus-css.md) — Status `done`.
3. [18 — AI Actor stream](issues/18-ai-actor-stream.md) — Status `defined`. Blocked by [17 — CloudAgents Console stream](issues/17-cloudagents-console-stream.md).
4. [17 — CloudAgents Console stream](issues/17-cloudagents-console-stream.md) — Status `defined`.
5. [16 — AiRepos from appsettings](issues/16-airepos-from-appsettings.md) — Status `done`.
6. [15 — AiKeys from appsettings](issues/15-aikeys-from-appsettings.md) — Status `done`.
7. [14 — Provider-named AI errors](issues/14-provider-named-ai-errors.md) — Status `done`.
8. [13 — Vertical proof: Browser Ask from what I see](issues/13-vertical-proof-browser-ask.md) — Status `done`.
9. [11 — Pack extract with Amb (supplied-fragment walk)](issues/11-simple-extract-format.md) — Status `done`.
10. [08 — Agent ask from what I see](issues/08-agent-ask-from-what-i-see.md) — Status `done`.
11. [09 — Agent failure preserves children](issues/09-agent-failure-preserves-children.md) — Status `done`.
12. [10 — Cancel by Focus](issues/10-cancel-by-focus.md) — Status `done`.
