...
8. **Focus vs Command on Run** — Focus is the reply parent (Children replace/stream boundary). Command is the `?…` owner-ancestor that selects the Actor. They may differ: e.g. Focus = `What time is it?`, Command = `?ai cursor`. Browser one-Node Run (Command = Focus = Zoom) remains the hello/TestActor proof only. Product AI Run sends distinct ids; Client finds Command by owner-scan Focus → zoom root for text starting with `?`.
