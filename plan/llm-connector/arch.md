...
8. **Focus vs Command on Run** — Focus is the reply parent (Children replace/stream boundary). Command is the nearest runnable owner-ancestor that selects the Actor. A node is runnable when its text starts with `?` **or** contains `=`. They may differ: e.g. Focus = `What time is it?`, Command = `?ai cursor`. Browser one-Node Run (Command = Focus = Zoom) remains the hello/TestActor proof only. Product Run sends distinct ids; Client finds Command by owner-scan Focus → zoom root and stops at the first runnable node — **do not skip** a `=` line to reach a `?` above it.
