...
- The Command Node's text is the dispatch. `?test hello` launches TestActor hello. `?ai ...` will launch the Agent Actor; payload details later. Command role, Kind, and CSS class do not select which Actor to run. Command Nodes do not create a separate provider instruction channel.
...
-  2026-09-11 — Restored the locked dispatch examples on this ticket (`?test hello`, `?ai ...`). This ticket owns those examples. [[plan/core-creation/issues/29-prove-testactor-hello.md]] is the hello implementation proof.
