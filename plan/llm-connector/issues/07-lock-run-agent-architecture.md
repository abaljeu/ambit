...
- Core resolves ActorName per that dispatch, constructs the extract from its current authoritative Graph, and validates exact membership, reachability, and launch admission at the typed Core boundary.
...
- Focus exclusivity is [[plan/llm-connector/issues/06-define-command-run-agent-redesign.md]]. Different Focus values may run concurrently even when their extracts overlap.
...
- The full vertical proof remains the later locked-vertical-proof item in [[plan/core-creation/issues/Implementation Planning and Record.md]].
