...
4. [ ] **Agent ask from Focus** — As a person, I want Command text `?ai` with optional args (args ignored for now) to launch the Run Agent Actor with mixed-format context and return Owned children under Focus, so that Ask from what I see works end-to-end. Locked behavior: [[issues/06-define-command-run-agent-redesign.md|06]]; not yet implemented.
