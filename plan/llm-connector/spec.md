...
1. **Command-text Run Agent** — The person marks a Command Node, grows included context under Zoom, sets Focus, and Runs. Command text dispatches the Actor (`?test hello` for TestActor; `?ai` plus optional keyname then optional reponame for the AI Actor). Focus is the stable reply parent.
...
1. [x] **Command-text dispatch** — As a person, I want Command Node text to select which Actor runs (`?test hello`, later `?ai` plus optional keyname then optional reponame), so that role, Kind, and CSS class do not invent a second dispatch channel. Locked: [[issues/06-define-command-run-agent-redesign.md|06]]. Delivered for TestActor hello via Core. Keyname: [15 — AiKeys from appsettings](issues/15-aikeys-from-appsettings.md). Reponame: [16 — AiRepos from appsettings](issues/16-airepos-from-appsettings.md).
...
4. [x] **Agent ask from Focus** — As a person, I want Command text `?ai` with optional keyname then optional reponame to launch the Run Agent Actor with mixed-format context and return Owned children under Focus, so that Ask from what I see works end-to-end. Locked behavior: [[issues/06-define-command-run-agent-redesign.md|06]]. Delivered: [[issues/08-agent-ask-from-what-i-see.md|08]]. CloudAgents document is write-only XML with Focus as css class `focus` on the extract copy: [19 — AI extract pack is XML with Focus cssClass](issues/19-ai-xml-pack-focus-css.md). Amb extract-walk from [11 — Pack extract with Amb (supplied-fragment walk)](issues/11-simple-extract-format.md) stays for Amb persist. Keyname selects `AiKeys`: [15 — AiKeys from appsettings](issues/15-aikeys-from-appsettings.md). Reponame selects `AiRepos`: [16 — AiRepos from appsettings](issues/16-airepos-from-appsettings.md).
...
5. **First pack** — [11 — Pack extract with Amb (supplied-fragment walk)](issues/11-simple-extract-format.md) writes Amb extract-walk for Amb persist. The CloudAgents document is write-only XML; Focus is css class `focus` on the extract copy ([19 — AI extract pack is XML with Focus cssClass](issues/19-ai-xml-pack-focus-css.md)). Mixed-format owning-codec stays tabled. Mixed-format stories above remain the product intent.
