...
**See also:** [[07-optional-poll-bootstrap-hash.md]], [[../reports/poll-hash-fallback-loop.md]], [[src/Client/Program.fs]], [[src/Shared/BootCache.fs]], [[src/Client/Update.fs]] `StateLoaded`
...
- [ ] HITL: one `/state` then Poll confirms; Selection must not jump to ROOT.
...
- 2026-09-02: Parked from WORK.md. HITL: one `/state` then Poll confirms; Selection must not jump to ROOT.
