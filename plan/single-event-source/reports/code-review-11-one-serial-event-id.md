...
Markdown / planning: no consecutive-blank or bare-id hits ([markdown-writing.md](.agents/rules/markdown-writing.md), [refer-by-name.md](.agents/rules/refer-by-name.md)). Ticket and [project.md](plan/single-event-source/project.md) use `[label](path)`. [arch.md](plan/single-event-source/arch.md) keeps allowed `[[path]]` bare refs. Plan text still says “interject”; that wording is stale. The code is server merge / revised stream (`undoPendingGraph` / `consumeCatchUpPoll`). Stage stays `build`; ticket Status `coded`. `type Revision` is [12 — Contract leftover Change and Revision](plan/single-event-source/issues/12-contract-leftover-change-and-revision.md), out of scope.
