...
9. **Browser Run** — [[src/Client/Commands.fs]]
   ...
   - Interface:
     ...
     3. [x] Product Run (AI and later Actors): Focus is the selection reply parent (may differ from Command). Command is the nearest runnable owner-ancestor: text starts with `?` **or** contains `=` (scan Focus → zoom root; stop at the first; do not skip `=` lines). Zoom root stays the Included extract root. Send distinct `focusId`, `commandId`, and `zoomId`. Actor writes under Focus (replace or stream children per llm-connector).
