...
5. **EventLog** — planned [[src/Shared/EventLog.fs]]
   ...
   - Interface:
     ...
     9. [x] EventLog persist is authoritative. Restart re-executes Ops `Ev` records onto Graph when Graph/projection lagged; ActorStart/ActorStop restore into EventLog only. `getEventId` / `State.eventId` / HTTP `latestId` are the EventLog tip. [49 — Mailbox History durability](issues/49-mailbox-history-durability.md)
   - Uses:
     1. [ ] Ev
...
12. **PersistHandlers** — [[src/Server/Core/FileAgent.fs]], [[src/Server/Core/DbAgent.fs]]
   ...
   - Interface:
     ...
     4. [x] On create, replay Ops `Ev` records from the persisted EventLog onto Graph when the Graph checkpoint lags; then `getEventId` is the EventLog tip. Actor bodies do not apply. [49 — Mailbox History durability](issues/49-mailbox-history-durability.md)
   - Uses:
