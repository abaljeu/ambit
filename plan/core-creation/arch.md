...
12. **PersistHandlers** — [[src/Server/Core/FileAgent.fs]], [[src/Server/Core/DbAgent.fs]]
   ...
   - Interface:
     ...
     4. [x] On create, run EventLog.recover: Graph id greater than Log id drops the log; equal is noop; Log id greater than Graph id applies Events until concurrent. EventId moves only via apply. Actor bodies do not apply. [46 — Mailbox History durability](issues/46-mailbox-history-durability.md)
   - Uses:
