...
- 2026-09-14 — Implement §3 CoreActorPool start: Created LoadedDescendantIds.expand for graph expansion from Zoom root; created TestActor module with actorFn that interprets command node text; updated ActorFn signature to accept ActorInput with named ids and secret; implemented CoreActorPool.startActor to expand graphIds, select actor from command node, append ActorStarted via callback, and schedule actor body as fire-and-forget; updated CoreMailboxBackend to pass getState, coreChanges, and appendActorStarted callback to pool.startActor; registered TestActor in CoreRuntime.create; added tests proving expand/select/live-row-before-schedule order and ActorStarted recording via lifecycleEvents.
