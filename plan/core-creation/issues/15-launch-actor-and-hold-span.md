...
- [ ] Launch with registered name, Revision, parent NodeId, and non-empty span starts an Actor and returns a never-reused public number.
- [ ] The Command caller does not receive the send credential; the Actor does, and that credential is in the Core set.
- [ ] Launch that shares any NodeId with a live span is refused.
- [ ] Live Nodes in the span show lock-present; SQL and History do not carry lock.

## See also

[[09-define-core-command-launch-contract.md]], [[plan/core-creation/reports/to-build-09-12-actor-pool.md]]
