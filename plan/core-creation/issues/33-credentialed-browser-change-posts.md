...
1. [ ] Browser Change submit — existing Change submit supplies Authority and secret on the post
2. [ ] No new Browser chrome — reuse existing Change UI; add no new control

### 2. HTTP Adapter

Transport encoding. Contracts on arch **HTTP Adapter**.

1. [ ] Encode Change + credentials — decode Browser Change; Change posts carry credentials
2. [ ] Pass into Core door — pass Authority and secret into CoreMailbox `postChange`
3. [ ] Adapter stays decode and status — admission is not re-implemented in the Adapter

### 3. CoreMailbox

Public door. Contracts on arch **CoreMailbox**; Seam **CoreMailbox door**.

1. [ ] Credentialed `postChange` — public `postChange` takes Change list plus Browser Authority and secret (same door Browser and Actor will share; this ticket exercises Browser only)
...
1. [ ] `PostChange` carries credentials — every `PostChange` (Browser path here) requires Authority and secret on the message
2. [ ] Validate before PersistHandlers — CoreMsg validates Browser credentials before calling PersistHandlers
3. [ ] Live admits — a live Browser credential is admitted and the Change reaches PersistHandlers
4. [ ] Inactive refuses — a missing or inactive credential is the same auth refuse; PersistHandlers is not called for that refuse
5. [ ] No Actor live-table admit — this ticket does not require CoreActorPool live-row admit (that stays with hello Story paths)
