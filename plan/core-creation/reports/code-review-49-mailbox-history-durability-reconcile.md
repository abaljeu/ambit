...
4. **Hard — `change` bound to Ev.** [fsharp-source.md](../../../.agents/rules/fsharp-source.md): rename the local to `event` / `events`. [MailboxHistoryDurabilityTests.fs](../../../tests/Server.Tests/MailboxHistoryDurabilityTests.fs) `stop :: change :: start` and `let childId, change`. [EventTests.fs](../../../tests/Shared.Tests/EventTests.fs) `let change, childId = recoverChange`.
