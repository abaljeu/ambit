# 46 — Mailbox History durability reconcile

Date: 2026-09-19. Ticket: [46 — Mailbox History durability](../issues/46-mailbox-history-durability.md). Status stays `coded`.

This is a correction note, not authority. Alan’s load reconcile supersedes empty→zero and max-tip patches ([46 mailbox History durability empty-log tip](46-mailbox-history-durability-empty-log-tip.md), [46 mailbox History durability live-append tip](46-mailbox-history-durability-live-append-tip.md), [code-review-46-mailbox-history-durability-rereview](code-review-46-mailbox-history-durability-rereview.md) Spec (c)).
...
5. [File Graph greater than Log drops persist EventLog](../../../tests/Server.Tests/Issue46MailboxHistoryDurabilityTests.fs) and the Db twin — persist log gone; `getEventId` is Graph.
6. [File equal Log and Graph keeps EventLog](../../../tests/Server.Tests/Issue46MailboxHistoryDurabilityTests.fs) — log stays.
7. [File empty Graph checkpoint live append continues past Graph](../../../tests/Server.Tests/Issue46MailboxHistoryDurabilityTests.fs) — next mint is past Graph; no max bandage.
