# 49 — Mailbox History durability empty-log tip

Date: 2026-09-19. Ticket: [49 — Mailbox History durability](../issues/49-mailbox-history-durability.md). Status stays `coded`.

Superseded by [46 mailbox History durability reconcile](49-mailbox-history-durability-reconcile.md). Empty→zero is not the door rule. Graph-ahead does not keep a lagging log or max tips.

This is a correction note, not authority. Alan overruled [code-review-49-mailbox-history-durability](code-review-49-mailbox-history-durability.md) Spec (c)1 (empty EventLog → `EventId.zero`).
...
3. [File empty EventLog recover exposes getEventId Graph checkpoint](../../../tests/Server.Tests/MailboxHistoryDurabilityTests.fs) — `gambol.meta` 4, empty EventLog, `getEventId` is 4.
