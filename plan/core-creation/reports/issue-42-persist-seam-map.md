...
5. **postGraphOnlyChange** — `Change list -> Result<CoreChangesAccepted, string>` (graph apply + EventLog append; skips file persistence only)
...
3. **CoreMailboxBackend** — [[src/Server/Core/CoreMailboxBackend.fs]]. `MailboxContext.persist` is the seam. `GetState` / `GetRevision` / `GetChangesSince` / `SnapshotDone` call handlers directly. `PostGraphOnlyChange` → **CoreEventDispatch.postEvent** with `graphOnly=true` (appends to EventLog, calls `persist.postGraphOnlyChange` instead of `persist.postChange`). `PostEvent` → **CoreEventDispatch.postEvent** with `graphOnly=false` (may call `persist.postChange`). `StartActor` / `ActorStop` append lifecycle Events in memory only (see §4). Startup `failedPersist` keeps `getState` / `getRevision` / `getChangesSince` and closes `postChange` / `postGraphOnlyChange`.
4. **CoreEventDispatch** — [[src/Server/Core/CoreEventDispatch.fs]]. `Context.persist` used only when `Event.ops` is `Some`: builds a `Change` from Ops + `submissionId` and calls `persist.postChange [ change ]` (or `persist.postGraphOnlyChange [ change ]` when `graphOnly=true`). ActorStart / ActorStop bodies have `ops = None`, so they never reach PersistHandlers here.
