...
- [46 — Mailbox History durability](issues/46-mailbox-history-durability.md) — EventLog-authoritative durability: persist/load the audit sequence, re-execute Ops `Ev` records when Graph lagged, one serial (`getEventId` / `latestId` = EventLog tip). Status `coded`. Blocked by [35b — Browser Run hello](issues/35b-browser-run-hello.md). Not required for 35b §7 Browser proof. Plan: [46 mailbox History durability explore](reports/46-mailbox-history-durability-explore.md). Implement: [46 mailbox History durability](reports/46-mailbox-history-durability.md).
