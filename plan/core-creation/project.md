...
- 2026-09-20 — Filed [[issues/50-actor-live-labels-from-command.md|50 Actor live result labels from Command node]]: `?test` wrongly labeled AI; Command id = scan up from Focus to zoom root for text starting with `?`. Status `defined`.
...
- [50 — Actor live result labels from Command node](plan/core-creation/issues/50-actor-live-labels-from-command.md) — Bug: `?test` live results labeled AI; resolve Command by scanning up from Focus to zoom root for `?` text. Status `defined`.
