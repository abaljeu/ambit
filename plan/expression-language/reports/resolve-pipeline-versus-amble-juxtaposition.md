# Resolve Pipeline versus Amble juxtaposition

Recorded the HITL decision on [[plan/expression-language/issues/01-pipeline-versus-amble-juxtaposition.md|Pipeline versus Amble juxtaposition]]. Status is resolved. CONTEXT.md was left unchanged.

## Changed

- Ticket Answer: one surface; juxtaposition left-associative (APL reverse); space only lexes; `#todo` is `#` plus `todo` meaning tagged; anchors, postfix, and infix; `text` is postfix (`Ref text`); omitted left side is the current Node.
- Map [[plan/expression-language/map.md]] Decisions so far: one gist line, title wrapping the wikilink.
- [[plan/expression-language/issues/03-first-primitive-catalog.md|First primitive catalog]] `Blocked by:` is now `02` only. [[plan/expression-language/issues/04-boolean-operators-as-control.md|Boolean operators as control]] is unblocked.
- Pointer sentences on [[plan/expression-language/issues/02-path-references-as-pipeline-terms.md|Path references as pipeline terms]] and First primitive catalog. Status of those tickets unchanged.
...
- [[plan/expression-language/issues/02-path-references-as-pipeline-terms.md|Path references as pipeline terms]]
