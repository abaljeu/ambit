# Resolve Statements in this spec

Recorded the HITL lock on [[plan/expression-language/issues/07-statements-in-this-spec.md|Statements in this spec]]. Status is resolved.

## Changed

- Ticket Answer: `= Expression` and `Name=Expression` (plus rename) in; bare Expression forbidden (Run does nothing); `#ident =` rejected; `>` not in this spec. Valid `=` with 0 Answers or type error: blueletter `No matches found`. Unfold when Children are written. Dialog: leading `=` evals; else word search.
- [[plan/expression-language/issues/06-top-level-context-node-versus-text.md|Top-level context]] Answer: redletter fail and always-on language matcher revised; points at 07.
