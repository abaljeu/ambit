...
- 2026-08-29 Q2: Same Run command, third statement form `?` plus a message (example: `? Based on the visible nodes (included context) what should i do next`). Not a sibling command. Spec today: Run is only `=` / `Name=` ([[.scratch/expression-language/spec.md]] ch. 8).
...
- 2026-08-29 Q6: New feature-set Project owns pack, LLM call, write-back. Expression-language only recognizes `?`. Long-running (stop a slow bot; answers while the person works) already lives on [[.scratch/event-sourced-ops/]]: [[details/actors-and-jobs.md]], [[issues/07-generalized-server-actor-produce-path.md]], [[issues/09-job-identity-with-advisory-soft-lock.md]].
