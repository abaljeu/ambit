...
- [ ] Live tickets use only `ready-for-agent` | `ready-for-human` | `needs-info` | `blocked` | `done` | `cancelled`, with bold field labels.
- [ ] `open` and the one `in-progress` ticket are takeable (`ready-for-agent` unless clearly human). `resolved` / `closed` / `agent-done` are `done`. Any `wontfix` is `cancelled`.
- [ ] Other Feature-set Projects use locked Stage tokens (`charting` → `chart`, `tickets` → `slice`, `active` → `build`). The Roadmap Project file has no Stage.
- [ ] The inventory search class in [[plan/skills-cleanup/reports/status-reconsider-inventory.md]] is re-run. Live tickets show no forbidden first tokens.
