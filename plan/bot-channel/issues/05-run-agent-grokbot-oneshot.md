...
1. [ ] First token `gbot` — `?ai gbot` (extra tokens ignored) uses `GrokBotRunner` wake + `streamUntilComplete` + cancel
2. [ ] Other behaviors — keep Cursor `AgentRunner` (cursor path unchanged)
3. [ ] One Actor name — `ai`; no second registration
4. [ ] Shared FocusXmlStream fold — one `AssistantText` / `RunFinished` path for both backends

### 2. Grok oneshot on the existing runner

1. [ ] Pack extract — same `AiExtractPack` as cursor
2. [ ] Wake — `GrokBotRunner.wake` with pack + `commandId` + `focusId` + Actor-minted oneshot `sessionId` (not pool deliver)
3. [ ] Stream — `GrokBotRunner.streamUntilComplete` + the shared FocusXmlStream fold
4. [ ] Finish — `RunFinished` is the same class of terminus as cursor (`ActorSucceeded` / live Focus gone)
5. [ ] Cancel — mid-stream abort via `GrokBotRunner.cancel`; no success Finish; no close-notify
6. [ ] Empty `WakeUrl` — failure surfaces safely (`ActorFailed`); secrets do not land in Graph

### 3. Composition bind

1. [ ] Bind `GrokBotConfig` from User Secrets / config `grokbot:WakeUrl` / `WakeSecret` / `InboundSecret` at composition
2. [ ] Library stays settings-blind
3. [ ] Do not invent inbound body fields in Server ([06 — Done seam for response concluded](06-done-seam-response-concluded.md) stays Unsettled under the Grok adapter)
...
1. [ ] Fake Grok stream → Focus growth / Finish class of terminus
2. [ ] Cancel mid-stream drops live and keeps streamed children
3. [ ] Empty `WakeUrl` failure surfaces safely (no secrets in Graph)
4. [ ] Cursor `?ai` path unchanged
