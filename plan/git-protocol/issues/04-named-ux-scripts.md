...
- 2026-09-02: Filed from [[plan/git-protocol/scripts-spec.md]]. Spec typos (`whate`, `"dev"` vs `"desc"`, `forward dev`) mapped to current [[scripts/merge.sh]] `forward` behavior. This issue does not edit instruction skills or retire the old script names.
- 2026-09-02: Implemented. Named wrappers call [[scripts/merge.sh]] and [[scripts/push.sh]]. [[scripts/commit.sh]] with no argument runs `git status`. Live pointers in [[.cursor/skills/git-protocol/SKILL.md]], [[.cursor/skills/git-master/SKILL.md]], [[.cursor/skills/git-share/SKILL.md]], and [[CONTEXT.md]] now name the new scripts. [[.cursor/skills/git-share/SKILL.md]] still uses `merge.sh forward ready` because [[scripts/gitdev.sh]] takes no dev. Did not delete the old scripts. Did not change [[plan/git-protocol/project.md]] or [[plan/index.md]].
