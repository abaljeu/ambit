...
3. Run **`./scripts/merge.sh ready`** to bring `dev` into `ready` (`--no-ff`).
...
5. **Later** squashes onto `master` go through **`./scripts/merge.sh master -m "<message>"`** per [[.cursor/skills/git-master/SKILL.md]]. Publishing `master` fast-forwards to `origin` once step 2 is in the ancestry.
