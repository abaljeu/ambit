...
| VS Code task name | work-line version named a task **desktop: Run**, which does not exist | now names **desktop: Run (cloud)** and **desktop: Run (local)** | [[.vscode/tasks.json]] labels are `desktop: Run (cloud)` and `desktop: Run (local)`; `scripts/desktop.sh run` is still a valid action per the script usage text |
