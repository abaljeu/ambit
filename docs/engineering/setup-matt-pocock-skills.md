...
- **Domain docs** — assumed single-context (one `CONTEXT.md` + `docs/Decisions/` at the root), which fits almost every repo; it only raises a multi-context map when it spots monorepo signals.
