...
|------|----------------|--------|
| HEAD (`dev`) | `5db017c` — "Reconcile public README and doc with cPanel proxy; drop legacy deploy files." | Single landing + developer README: Status first, full architecture (Desktop, Npgsql), `/ambit` on port 5215, npm/bundle, Persistence, Desktop, cPanel proxy links. Documented in [[.scratch/git-protocol/reports/public-face-merge.md]]. |
