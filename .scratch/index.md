...
| [Login / context restore](login-context-restore/) | charting | After Safari discards an inactive Gambol tab, return already authenticated and with UI context (e.g. zoom) restored — wayfinder map in progress. |
...
| [Parse load demote](parse-load-demote/) | active | Keep Current documentState when Browser Load Workspace rediscovers an already-Parsed file as Added; demote only new stubs and NoServerFile→present. |
