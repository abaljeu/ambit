...
- Poll and post-bootstrap load responses use the same catch-up transaction from client base revision `B` to one atomically read server revision `R`. It contains the ordered changes from `B` through `R` plus authoritative headers or tombstones at `R` for every node touched or named by that tail. A load response also contains its requested `Direct | ArtifactClosure | Workspace` snapshot at `R`; normally `B = R` and the tail is empty.
- Tail changes project onto facts represented before the transaction. Structural spans apply only to `Loaded` lists. Supplemental headers install only when already resident or required by a resident or newly loaded list; other facts remain absent. Requested snapshots authoritatively overwrite their returned headers and lists at `R`.
- The client applies tail projection, supplemental facts, requested snapshots, derived indexes, and revision advancement atomically. A server tombstone removes a represented deleted node; deletion is not residency eviction. Missing history needed after `B` requires a full bootstrap.
