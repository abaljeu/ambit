...
- Ordinary undo and redo load nothing: every dependency resident when the original change was built remains resident for the webpage session. How history reverses a server-expanded permanent Delete is tentative pending [Finalize permanent Delete undo semantics](13-finalize-permanent-delete-undo.md); it must not force hidden parent lists to become resident.
