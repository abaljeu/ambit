...
The corrective restart landed in commit `4255c48` (“implementing new undo plan ready for partially downloaded graph”). Ticket 15 is **agent-done**: `ChangeRequest` is the pending-queue / submit / server-command unit; ChangeLog, Poll, bootstrap, and Load remain Change-only.
