...
**What to build:** Add Shared tests (or an equivalent harness) that exercise the chapter 11 table: valid Expressions with expected Answer sets or outcomes, plus parse-error, type-error, and zero-Answer rows. Cover structural desugar (`//ws`, `d/e`), content search (`d#e`, `a#b#c`, `^#blue`), pure filters (`/ "d" dir`, `class "h1"`), combinators (`#x , #y`, `NOT containing`), and consumer-relevant forms where testable in Shared. Each row asserts the spec outcome, not today's RefExpr behavior.
