...
- [ ] `//ws` and `root /ws` parse to the same cluster steps as `root / "ws"`; bare `//` and bare `/` are parse errors.
- [ ] `a/b/c` parses as implicit `/ "a"`, then `/ "b"`, then `/ "c"` from the omitted left input.
- [ ] `:*` and `!*` parse as child-all and sibling-all steps; `3` alone is a parse error.
- [ ] `x / "filename with spaces"` and `x # "a b"` accept spaced quoted name arguments.
