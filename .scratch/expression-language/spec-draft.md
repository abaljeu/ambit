...
[[.scratch/expression-language/issues/07-statements-in-this-spec.md|Statements in this spec]]. First spec: `= Expression` and `Name=Expression` (rename plus materialise). Bare Expression is not a Run statement (do nothing). `#ident = Expression` is rejected. `>` shell is not in this spec. Valid `=` / `Name=` with 0 Answers or type error: blueletter Child `No matches found`. If Run writes Children, unfold that Node. Search/Move: leading `=` evals a Node Expression locally; otherwise today’s word search.
...
[[.scratch/expression-language/issues/06-top-level-context-node-versus-text.md|Top-level context: Node versus text]]. Every function has a defined type. Mixing kinds via `AND` / `OR` / `NOT` is an error. No number producers in this catalog. A number literal is only valid as the right operand of `:` or `!`. Run: Node Answers become Ref Children; text Answers become new Owned Nodes. Search and Move keep the existing scrolling dialog; a leading `=` evals a Node-typed Expression locally (omitted left is zoomRoot); otherwise today’s word search; pick one Node (Search zooms, Move relocates). 0 Answers and parse/type error both show no hits. Statement form, no-op, blueletter, and unfold: [[.scratch/expression-language/issues/07-statements-in-this-spec.md]]. No display consumer.

## Unloaded walks — Locked

[[.scratch/expression-language/issues/13-fog-of-the-first-spec.md|Fog of the first spec]]. A walk that meets an Unloaded Node yields no Answer. It does not Load. All eval is local. Server-side Search is postponed: [[.scratch/expression-language/issues/14-server-side-search.md|Server-side search]].

## Examples — Confirmed prototype (not locked syntax)

[[.scratch/expression-language/reports/pipeline-examples.md]] HITL 2026-08-27 ([[.scratch/expression-language/issues/08-prototype-pipeline-examples.md|Prototype: pipeline examples]]): rows match, then amended. `//ws/x` may be a Workspace Node or Directory Node named `ws`. `#blue` defers to locked `#` search details. `// OR /` is undefined (`/` is not a prefix). Bare `3` is a type error. `child` / `descendant` / `tree` as on [[.scratch/expression-language/issues/12-owned-versus-ref-walk-for-descendant.md|Owned versus Ref walk for descendant]].

## Prolog control — Research resolved

[[.scratch/expression-language/issues/09-research-prolog-control-mapped.md]]: conjunction, disjunction, and negation-as-failure map as control over an Answer stream. Collection is a context rule. Cut, if-then, unification, and `bagof` grouping are not planned ([[.scratch/expression-language/issues/13-fog-of-the-first-spec.md|Fog of the first spec]]). Note: [[.scratch/expression-language/reports/prolog-control-mapping.md]].

## Seams the spec must not contradict — Research resolved

[[.scratch/expression-language/issues/10-research-amble-refexpr-seams.md]]: empty miss is fail-to-answer, not silent success; parse errors stay distinct. Pipeline space must share operators with Amble juxtaposition. Amble `,` stays concatenate. Find AND is not the pipeline. Implemented `**` is Owned-only and stops at Directory/Workspace; this spec’s `**` is `tree` and does not stop there. Other path steps follow Ref. Note: [[.scratch/expression-language/reports/amble-refexpr-seams.md]].
