...
- Two Project kinds only: this Roadmap, and feature-set Projects (existing `.scratch/<slug>/`). No Epic Project folders. Standing Epic files live in [[epics/]] (`epics/<slug>.md`), one file per Epic, Chapters in that file. [[issues/]] is wayfinder tickets only (this map’s decisions and tasks). The frontier does not scan `epics/`.
