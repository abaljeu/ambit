...
module SyncLogic =
    ...
    let getPollOutcome
        ...
        let pollEventId = EventId.value poll.eventId
        let dataOutdated = pollEventId > clientEventId
    ...
    let applyLoadResponse
        ...
        let stateEventId = EventId.value state.eventId
        if
            packageOnly
            && (hasPendingLocal || responseEventId <> stateEventId)
        then
