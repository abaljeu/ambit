...
- Reference / path syntax stays `//` (ROOT) and `//workspacename/...`. No colon address form; no `:` as workspace address punctuation.
...
- Any production DataDir migration, rename, or delete of local legacy marker-prefixed folders under `data/` (ignore them).
...
| [[src/Shared/DocumentAssembly.fs]] | marker-prefix artifact heuristic; `TrimStart '@'` on stubs |
...
Tests still assume marker-prefixed names / colon-address queries in RefExpr, Search, ViewModelFileSearch, and disk/persistence suites — addressed in slices below.
...
5. Update disk/persistence test expectations from marker-prefixed `home` folder names to bare `home` (no WorkspaceGit/Connect tests — absent at restart).
6. Verify: Shared.Tests + Server.Tests DocumentPersistence / DocumentPathMoveExecution green.

### Slice B — Refs and docs without colon address form — done
...
- Immutable names.
- Local legacy marker-prefixed folder cleanup under `data/`.
