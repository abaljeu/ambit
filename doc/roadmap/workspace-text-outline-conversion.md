...
Import/warm reconcile for outline-backed text formats uses a shared mechanism in `src/Shared/documents/` (`OutlineLcs` + `OutlineReconcile` + `OutlineDocument.warmByLcs`, DiffPlex-backed). Formats project into a nested `SpanNode` tree (parent encloses children); LCS runs over preorder bound lines. See [[doc/reference/formats/code-shape.md]].
