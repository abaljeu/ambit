...
- After delete/rename/move reconciliation is implemented, a git-deleted workspace file trashes its graph node under `//TRASH/`; the workspace disk path is gone and the UI shows a missing target. The user can recover from TRASH in the graph or from Git history on disk. Moving out of TRASH may recreate disk content if the user chooses.
...
successful git-receive-pack
  ...
  → extract changed workspace-relative paths (A / D / R; ignore M)
...
## Implemented capability: create-only post-receive reconciliation

After a successful `git-receive-pack`, the server compares the old and new `HEAD` and extracts added paths. An initial push with no old `HEAD` treats every path in the new tree as added. Missing Directory ancestors and the File stub are created under the workspace named by the gateway label. Existing matching stubs make repeated reconciliation idempotent.

The server submits the planned ops through the active graph agent, which uses FileAgent in file mode and the authoritative DbAgent in database mode. This preserves revision/change-log behavior and makes the result visible through existing polling. No file content is read or parsed.
...
## Planned capability: delete, rename, and move reconciliation

Extend post-receive reconciliation beyond create-only (`LazyLoadReconciliation.planAddedPaths`). **Out of scope for this slice:** content parse/merge, freshness (`M`), repair/retry policy, client pull.
...
| `M` path | Ignore for structural reconciliation (freshness later) |
...
**Apply order:** deleted (deepest first) → renamed/moved → added.
...
1. `WorkspaceGit.changedPathsBetween` — `A` / `D` / `R` extraction + server tests.
...
5. `planChangedPaths` — orchestrate delete → rename → add; extend `LazyLoadReconciliationServer.reconcileAddedPaths` → `reconcileChangedPaths`.
6. Snapshot/persist behavior — structural lazy-load reconciliation must not write workspace file bodies; path move no-ops when receive already removed the source.
...
2. **Disk-to-graph stub reconciliation** — create-only added-path handling is implemented. Delete, rename, and move reconciliation is specified above (locked semantics and implementation steps); this is the next implementation increment.
3. **Expand-to-parse and freshness** — parse one File on expansion, merge into its existing identity, and carry current/unparsed/older/newer metadata.
...
- Pull/freshness, expand-to-parse, deletes/renames/moves, document load units, and residency/search are independent capabilities.
...
- Add expand-to-parse, source metadata, and freshness UI.
- Introduce document membership and load/unload behavior.
